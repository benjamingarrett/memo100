/*
 * File:   linear_probe_test_3_byte_key_9_byte_value.c
 * Author: benjamin
 *
 * Created on Aug 12, 2015, 8:49:14 PM
 */
...
void test1() {
    ...
    #ifdef SHOW_PROGRESS
        printf("linear_probe_test_3_byte_key_9_byte_value test 1\n");
    #endif
...
*/
    ...
    if(successes==trials){
        printf("linear probe test 3 byte key 9 byte value - PASSED!\n");
    } else {
        printf("linear probe test 3 byte key 9 byte value - FAILED! (%d failures)", trials - successes);
    }
}
...
int main(int argc, char** argv) {
    ...
    #ifdef RUN_TEST
        printf("%%TEST_STARTED%% test1 (linear_probe_test_3_byte_key_9_byte_value)\n");
        test1();
        printf("%%TEST_FINISHED%% time=0 test1 (linear_probe_test_3_byte_key_9_byte_value) \n");
    #endif
    return (EXIT_SUCCESS);
}
